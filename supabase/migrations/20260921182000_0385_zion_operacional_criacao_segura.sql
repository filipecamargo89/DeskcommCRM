-- Zion Operacional: criacao atomica de proposta pelo backend.
-- A funcao e exclusiva do service_role. O ator e a organizacao sao
-- resolvidos pelo handler autenticado e revalidados aqui no banco.
...
begin
  ...
  -- Defesa em profundidade: nao confiar apenas na autorizacao do handler.
  -- Usuario comum precisa ser membro ativo agent/manager/admin da organizacao.
  -- Platform admin somente entra aqui quando possui scope full.
  if not exists (
    select 1
...
comment on function public.fn_create_credit_proposal(
  ...
  'Cria proposta do Zion Operacional de forma atomica pelo backend, revalida ator e organizacao e nunca retorna os campos criptografados.';
